...
class VisualServoingNode(Node):
    def __init__(self):
        ...
        # Optional marker offset from detected center
    ...
    def marker_callback(self, msg: MarkerPointStamped):
        ...
        lin_vel = self.k_lin * (-error_x)
        ang_vel = self.k_ang * error_y

        # Clamp velocities
        cmd = Twist()
        cmd.linear.x = max(min(lin_vel, self.max_lin_vel), -self.max_lin_vel)
        cmd.angular.z = max(min(ang_vel, self.max_ang_vel), -self.max_ang_vel)
