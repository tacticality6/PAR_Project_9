...
class VisualServoingNode(Node):
    def __init__(self):
        ...
        self.color_image_sub = self.create_subscription(CompressedImage, self.colourImageTopic, self.color_callback, qos_profile_sensor_data)
        
        # Subscribing to the raw depth topic, expecting an uncompressed Image message
        self.depth_image_sub = self.create_subscription(Image, self.depthImageTopic, self.depth_callback, qos_profile_sensor_data)

        self.camera_info_sub = self.create_subscription(CameraInfo, self.cameraInfoTopic, self.camera_info_callback, qos_profile_sensor_data)
    ...
    def color_callback(self, msg):
        # Correctly handles compressed color images
        np_arr = np.frombuffer(msg.data, np.uint8)
        self.color_image = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)

    def depth_callback(self, msg):
        # Correctly handles UNCOMPRESSED raw depth images
        try:
            # Use cv_bridge for uncompressed images. 'passthrough' preserves the 16-bit depth data.
    ...
    def localise_pointer(self):
        ...
        # This check now works because self.depth_image is a 2D array of single values
        ...
        try:
            ...
            self.get_logger().info(f"Successfully localized pointer offset in base_link: {self.marker_offset}")
        except Exception as e:
            self.get_logger().error(f"TF transform failed while localizing pointer: {e}")

    def marker_callback(self, msg):
        if self.state == VisualServoingState.IDLE: return
        if self.marker_offset is None: return
        try:
